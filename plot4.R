plot4 <- function()
{
	...
	plot(subset1[,1], subset1[,3], ylab = "Global Active Power", xlab = "", main = "", pch = ".")
	lines(subset1$Date,subset1$Global_active_power)
	
	plot(subset1[,1], subset1$Voltage, xlab = "datetime", main = "", pch = ".")
	lines(subset1$Date,subset1$Voltage)
	
	plot(subset1[,1], subset1$Sub_metering_1, ylab = "Energy sub metering", xlab = "", main = "", pch = ".")	
	lines(subset1$Date,subset1$Sub_metering_1)
	lines(subset1$Date,subset1$Sub_metering_2, col = "red")
	lines(subset1$Date,subset1$Sub_metering_3, col = "blue")
	legend("topright", legend = c("Sub_metering_1", "Sub_metering_2", "Sub_metering_3"), col = c("black", "red", "blue"), pch = "-")
	
	plot(subset1[,1], subset1$Global_reactive_power, xlab = "datetime", main = "", pch = ".")
	lines(subset1$Date,subset1$Global_reactive_power)
